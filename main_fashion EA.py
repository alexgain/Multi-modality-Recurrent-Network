...
class Net(nn.Module):
    def __init__(self, adj_net = None, adj_in = None, W_net = None, W_in = None, dna = None, W_dna = None, ff_net = None):
        ...
        self.W_dna = torch.Tensor(net_size**2,10).uniform_(0, 1)
        self.dna = torch.Tensor(10).uniform_(0, 1)

        #changing to dna stuff to nn.Parameters/Variables:
        if cuda_boole:
            self.W_dna = Variable(self.W_dna.cuda(), requires_grad = False)
            self.dna = Variable(self.dna.cuda(), requires_grad = False)
        else:
            self.W_dna = Variable(self.W_dna, requires_grad = False)
...
##        print(self.relu(self.W_dna.matmul(self.dna)).view(net_size,net_size).shape)
        self.adj_net = normalize_01(self.relu(self.W_dna.matmul(self.dna)).view(net_size,net_size))
...
##            y = y.long()
    ...
    return -(1)*loss.cpu().data.numpy()[0]

def net_fit_fun(net, z):
    new_net = deepcopy(net)
    if cuda_boole:
        new_net.dna = Variable(torch.Tensor(z.cuda()))
    else:
        new_net.dna = Variable(torch.Tensor(z))
    return train(new_net)

###                    ###
### Digvijay's EA code ###
###                    ###

def create_breedsets(gen = None, rng = [0.0,1.0], N = 5, p = 10, vec_steps = 4, subset_size = 4, replace = False):
...
def fitfun(bred, net, N = 5, print_bool=False, Fun = np.linalg.norm):
    ...
    Fun2 = lambda z: net_fit_fun(net,z)
    
    scores = np.apply_along_axis(Fun2,1,bred)
    chosen = -1*np.hstack((bred,np.array([scores]).T))
...
nextgen = fitfun(breednewsets(create_breedsets()), my_net)

for k in range(100):
    print('Generation #:',k)
    cur_net = deepcopy(my_net)
    newgen = create_breedsets(nextgen)
    newbreedset = breednewsets(newgen)
    chosen = fitfun(newbreedset, cur_net)
    my_net = deepcopy(cur_net)
    my_net.dna = Variable(torch.Tensor(chosen[0]))
